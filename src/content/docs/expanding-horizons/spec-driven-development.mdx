...
## Playing the Long Game with Spec Driven Development
...
![Product development loop where requirements sit between finding an opportunity and building, and get adjusted from results](../../../assets/spec-driven-development/requirements-role-in-product-development.png)
...
Major features, external integrations, and subsystems each form their own distinct universes, connected by portals (contracts).
...
![The valid standing desk solution space drawn as a box in three dimensions: load capacity, height range, and lift speed](../../../assets/spec-driven-development/standing-desk-solution-space.png)
...
### Executable Tests — Enforcement at cost of Business Visibility
...
- **High noise:** Business constraints get buried among hundreds of routine implementation, utility, and UI tests.
...
Unlike REST APIs (OpenAPI) or database schemas, there is no official industry standard for spec files.
However, during my experiments, I ended up using requirement tables as tooling-friendly specification files.
...
- **Requires custom tooling:** Connections with test suites needs to be programmatically enforced to avoid code drift .
...
The codebase—or the broader system implementation—is simply one concrete solution residing within that bounded space.
...
I like to think of them as stable requirements.
...
```
---
schema: swmansion.com/spec-md/1.0
custom:
...
| `SPEC_MD__FRONTMATTER__ALLOWLIST` | A SPEC's frontmatter MUST NOT have properties other than the ones specified by this spec (schema, custom). | Avoiding collisions between user data and potential new properties in future versions. |
| `SPEC_MD__FRONTMATTER__SCHEMA_DECLARATION` | A SPEC published as a public contract SHOULD include `schema: swmansion.com/spec-md/1.0` in the frontmatter section. | A contract must be more explicit than an internal spec. |
...
- **Extract requirements interactively** during early phases by having an AI agent interview stakeholders, resolve ambiguities, and refine spec files.
