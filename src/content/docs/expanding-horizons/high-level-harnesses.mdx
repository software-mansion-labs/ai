...
This section is one level of abstraction higher—it covers tools and patterns that treat agents as a managable workforce.
...
You may also set up the agent(s) to respond to issues as they appear.
...
Agent behavior is defined in a `WORKFLOW.md` file in the repository alongside the code. The prompt and runtime settings for each agent run are versioned the same way you version a CI pipeline.
